...
import { View, Text, Button, Switch, StyleSheet, ScrollView, Image, TouchableOpacity } from 'react-native';
import Slider from '@react-native-community/slider';
import * as ImagePicker from 'expo-image-picker';
import { ColorMatrix, concatColorMatrices, colorMatrices } from 'react-native-color-matrix-image-filters';
...
export default function SettingsScreen() {
  ...
  const { textSize, setTextSize, profileImage, setProfileImage } = useTextSize(); // Get global text size from context
...
const dynamicStyles = {
  container: { ...styles.container, backgroundColor: transformColor(styles.container?.backgroundColor || '#EDEDED') },
  topSection: { ...styles.topSection, backgroundColor: transformColor(styles.topSection?.backgroundColor || '#7c2933') },
  bottomSection: { ...styles.bottomSection, backgroundColor: transformColor(styles.bottomSection?.backgroundColor || '#EDEDED') },
  text: { ...styles.text, color: transformColor(styles.text?.color || '#FFF') },
  buttonText: { ...styles.buttonText, color: transformColor(styles.buttonText?.color || '#FFF') },
  settingTitle: { ...styles.settingTitle, color: transformColor(styles.settingTitle?.color || '#000') },
  infoText: { ...styles.infoText, color: transformColor(styles.infoText?.color || '#000') },
};
  ...
  return (
    ...
    <View style={dynamicStyles.bottomSection}>
      ...
      <View style={styles.infoSection}>
        <Text style={styles.subSectionHeader}>Personal Information</Text>

        <Text style={dynamicStyles.infoText}>Birthday: {userData.birthday}</Text>
        <Text style={dynamicStyles.infoText}>Gender: {userData.gender}</Text>
        <Text style={dynamicStyles.infoText}>Account Type: {userData.accountType}</Text>
...
<Text style={styles.description}>
  Enable features optimized for wheelchair users / Offer directions that use elevators/escalators for persons with limited mobility.
</Text>

{/* Toggle for Color Blind Mode */}
<View style={styles.settingItem}>
  <Text style={[dynamicStyles.settingTitle, { color: 'white' }]}>Color Vision Deficient</Text>
  <Switch
    ...
    {['Deuteranomaly', 'Protanomaly', 'Tritanomaly'].map((type) => (
      ...
      >
        <View style={styles.radioCircle}>
          {colorBlindMode === type.toLowerCase() && <View style={styles.selectedRadio} />}
        </View>
        <Text style={[styles.radioText, { color: 'white' }]}>{type}</Text>
      </TouchableOpacity>
    ))}
  </View>
)}
